...
class CreateAdminTables extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create($this->config('database.users_table'), function (Blueprint $table) use($engine) {
            ...
            $table->engine = "{$engine} comment '管理-用户表'";
        });

        Schema::create($this->config('database.roles_table'), function (Blueprint $table) use($engine) {
            $table->bigIncrements('id');
            ...
            $table->engine = "{$engine} comment '管理-角色表'";
        });

        Schema::create($this->config('database.permissions_table'), function (Blueprint $table) use($engine) {
            $table->bigIncrements('id');
            ...
            $table->engine = "{$engine} comment '管理-权限表'";
        });

        Schema::create($this->config('database.menu_table'), function (Blueprint $table) use($engine) {
            $table->bigIncrements('id');
            $table->bigInteger('parent_id')->default(0)->comment('上级ID');
            $table->integer('order')->default(100)->comment('排序');
            ...
            $table->engine = "{$engine} comment '管理-菜单表'";
        });

        Schema::create($this->config('database.role_users_table'), function (Blueprint $table) use($engine) {
            ...
            $table->engine = "{$engine} comment '管理-用户角色表'";
        });

        Schema::create($this->config('database.role_permissions_table'), function (Blueprint $table) use($engine) {
            $table->bigInteger('role_id')->comment('角色ID');
            ...
            $table->engine = "{$engine} comment '管理-角色权限表'";
        });

        Schema::create($this->config('database.role_menu_table'), function (Blueprint $table) use($engine) {
            $table->bigInteger('role_id')->comment('角色ID');
            ...
            $table->engine = "{$engine} comment '管理-角色-菜单表'";
        });

        Schema::create($this->config('database.permission_menu_table'), function (Blueprint $table) use($engine) {
            $table->bigInteger('permission_id')->comment('权限ID');
            $table->bigInteger('menu_id')->comment('菜单ID');
            ...
            $table->engine = "{$engine} comment '管理-菜单权限表'";
        });
    }
